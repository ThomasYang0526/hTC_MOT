...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 10 16:03:16 2021
...
class GT:
    ...
    def get_gt_values(self):
        ...
        gt_tid = np.zeros(shape=(self.batch_size, Config.max_boxes_per_image, Config.tid_classes), dtype=np.float32)
        gt_tid_mask = np.zeros(shape=(self.batch_size, Config.max_boxes_per_image), dtype=np.float32)
        ...
        for i, label in enumerate(self.batch_labels):
            ...
            hm, reg, wh, reg_mask, ind, tid, tid_mask = self.__decode_label(label_class)
            gt_heatmap[i, :, :, :] = hm
            gt_reg[i, :, :] = reg
            gt_wh[i, :, :] = wh
            ...
            gt_tid[i, :] = tid
            gt_tid_mask[i, :] = tid_mask
                        
        return gt_heatmap, gt_reg, gt_wh, gt_reg_mask, gt_indices, gt_tid, gt_tid_mask
    ...
    def __decode_label(self, label):
        ...
        for j, item in enumerate(label): #原始座標
            ...
            if item[5] != -1:
                tid[j] = np_utils.to_categorical(item[5], Config.tid_classes)
                tid_mask[j] = 1
...
if __name__ == '__main__':
    ...
    tids_dict = sorted(tids_dict.items(), key=lambda x:x[0])
    # print(tids_dict)
    # print(len(tids_dict))